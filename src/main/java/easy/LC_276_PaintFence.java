...
Fail to thoroughly understand problem!!! The problem asks us to compute the number of ways for arranging the
painting which has no more than two sussive post share the same color. It means we were allowed to make two
neighboring posts share the same color!
Analysis:

The problem of asking how many ways to do something is usually very easy!
And it could always be solved through dynamic programming. You just need to carefully design the transitional
function according to characteristics or certain restrictions.
...
public class LC_276_PaintFence {
     ...
     * So, you can think, if n >= 3, you can always maintain these two situations, You either paint the same
     * color with the previous one, or differently.
     *
     * Since there is a rule: “no more than two adjacent fence posts have the same color.”
     *
     * We can further analyze:
     *
     * from 2.1, since previous two are in the same color, next one you could only paint differently, and it
     ...
     * from 2.2, since previous two are not the same, you can either paint the same color this time (diff*1)
     * ways to do so, or stick to paint differently (diff*(k-1)) times.
     * Here you can conclude, when seeing back from the next level, ways to paint the same, or variable same
     * would equal to diff*1 = diff, and ways to paint differently, variable diff, would equal to same *
     * (k - 1) + diff * (k - 1) = (same + diff) * (k - 1).
     * @param n
     * @param k
     * @return
     */
