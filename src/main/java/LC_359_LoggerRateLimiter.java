...
public class LC_359_LoggerRateLimiter {
    ...
    private Map<String, Integer> logTimestampMap = new HashMap<>();

    public boolean shouldPrintMessage(int timestamp, String message) {

        if(logTimestampMap.get(message) == null) {
            logTimestampMap.put(message, timestamp);
            return true;
        } else {
            int previousTimestamp = logTimestampMap.get(message);
            if(timestamp - previousTimestamp > 10) {
                logTimestampMap.replace(message, previousTimestamp, timestamp);
                return true;
            } else {
                return false;
            }
